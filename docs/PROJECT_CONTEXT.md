...
- 2026-02-17（资讯时间显示时区修复）
  - 新增统一时间格式化工具 `formatDateTimeCn`（默认时区 `Asia/Shanghai`），避免容器默认 UTC 导致展示时间偏差 8 小时。
  - 资讯流卡片时间显示改为固定使用北京时间格式化，保持“优先原文发布时间（`publishedAt`）”的语义不变。
  - 信源管理页中的镜像探测时间与“上次刷新时间”统一切换为北京时间显示，减少跨页面时间认知不一致。
